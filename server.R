...
shinyServer(function(input, output) {
  ...
  output$ds_table <- renderDataTable({
    ...
    if(is.null(H) == FALSE)
    {
      ...
      H.ds <- NULL
    ds.table <- ds.table[ order(-xtfrm(ds.table$PTZ),ds.table$`Well Number`), ]
    ds.table[is.na(ds.table)] <- "NA"
    incProgress(amount = 12/16)
...
if (inGroup == FALSE)
{
    output$data_table <- renderDataTable({
      ...
      mse.table <- mse.table[ order(-xtfrm(mse.table$PTZ), mse.table$Group), ]
      mse.table[is.na(mse.table)] <- "NA"
      mse.table
      })
}
